...
export async function selectProfileByProfileEmail(profileEmail: string): Promise<Profile|null>  {
	try {
		...
		const sqlQuery: string = "SELECT BIN_TO_UUID(profileId) as profileId, profileBio, profileHash, profileHometown, profileName, profileImage, profileStyle, profileEmail FROM profile WHERE profileEmail = :profileEmail"
